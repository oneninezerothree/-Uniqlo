<template>
	<div class="search">
		<div class="searchContent clear">
			<div class="nav">
				<ul class="navList">
					<li v-for="(item, index) in goodsList" :key="index" :class="{ active: index == idx }" @click="tab(index)" v-text="item.fenlei"></li>
				</ul>
			</div>
			<ul class="listContent" ref="listContent">
				<li v-for="(zhonglei, index1) in goodsList" :key="index1" v-show="idx == index1">
					<div class="commodityBox">
						<a v-for="(item, index) in zhonglei.a" :key="index" href="javascript:;" class="commodityList">
							<img :src="item.imgUrl" />
							{{ item.names }}
						</a>
					</div>
